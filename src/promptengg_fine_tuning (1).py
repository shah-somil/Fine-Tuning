# -*- coding: utf-8 -*-
"""promptengg_fine tuning.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Ozxpc9UXF1FKMLpJ5jplaihPJYrsxmLx

# Project: Fine-Tuning FinBERT for Financial Sentiment Analysis on Noisy Data

This notebook documents the process of fine-tuning a domain-specific transformer model (`ProsusAI/finbert`) for **financial sentiment analysis**, with a focus on handling a realistic, noisy dataset.

Our goal is to adapt the pre-trained FinBERT model to classify financial text ('positive', 'negative', 'neutral') using data from multiple sources with varying levels of rater agreement. We employ techniques like weighted training and hyperparameter optimization (HPO) to maximize performance.

**Pipeline:**
1.  **Dataset Preparation**: Loading, cleaning (via priority-based deduplication), and splitting a multi-source financial sentiment dataset, including the creation of a "gold" test set from high-agreement data. We train primarily on the comprehensive but noisy `S3_max` scenario.
2.  **Model Selection**: Justifying the use of `ProsusAI/finbert` and `distilbert-base-uncased`.
3.  **Advanced Fine-Tuning**: Implementing a custom `WeightedTrainer` to handle label noise during training on `S3_max`.
4.  **Optimization**: Running HPO to find the best configuration for FinBERT on the `S3_max` dataset.
5.  **Baseline Comparison**: Evaluating the zero-shot performance of both `finbert` and `distilbert`, and the fine-tuned performance of `distilbert`.
6.  **Final Evaluation**: Assessing the performance of the HPO-tuned FinBERT against baselines on the clean "gold" test set.
7.  **Error Analysis**: Understanding the fine-tuned FinBERT's remaining weaknesses.
8.  **Finalization**: Saving the optimized FinBERT model.
...
"""## 3. Experiment: Improving a Generic Model

Our core experiment focuses on taking a standard, widely-used transformer model, `distilbert-base-uncased`, which has *no prior financial knowledge*, and fine-tuning it specifically for our financial sentiment task using the high-quality `S1_clean` dataset.

### 3.1. Baseline Performance (Zero-Shot DistilBERT)

First, we need to establish how poorly the generic model performs *before* any fine-tuning. We'll evaluate `distilbert-base-uncased` directly on our "gold" test set using a zero-shot approach. We expect this performance to be significantly lower than a domain-specific model.
"""

# REVISED CELL: Evaluate Zero-Shot DistilBERT (Correctly Configured)
...
print("--- Evaluating Zero-Shot DistilBERT ---")
device = 0 if torch.cuda.is_available() else -1
num_labels = len(label2id)

# --- Load Model WITH Correct Labels FIRST ---
print("Loading DistilBERT model with custom label configuration...")
id2label_int_keys = {int(k): v for k, v in id2label.items()}
label2id_str_keys = {k: int(v) for k, v in label2id.items()}
model_zero_distil = AutoModelForSequenceClassification.from_pretrained(
    "distilbert-base-uncased",
    num_labels=num_labels,
    id2label=id2label_int_keys,
    label2id=label2id_str_keys
)
tokenizer_zero_distil = AutoTokenizer.from_pretrained("distilbert-base-uncased")
print("Model config id2label:", model_zero_distil.config.id2label) # Verify keys are ints

# --- Create Pipeline ---
print("Creating pipeline...")
distilbert_zero_pipe = pipeline(
    "text-classification", model=model_zero_distil, tokenizer=tokenizer_zero_distil,
    device=device, truncation=True
)

# --- Prepare Gold Test Data ---
test_df["label"] = test_df["label_str"].map(label2id) # Use global label2id
test_df_valid = test_df.dropna(subset=["label"]).copy()
y_true_gold = test_df_valid["label"].astype(int).values
texts_gold = test_df_valid["text"].tolist()

# --- Run Inference ---
print(f"Evaluating on {len(texts_gold)} test samples...")
pred_zero_distilbert = distilbert_zero_pipe(texts_gold, batch_size=32)
pipeline_label2id = {label.lower(): i for label, i in distilbert_zero_pipe.model.config.label2id.items()}
y_pred_zero_distilbert = np.array([pipeline_label2id[p["label"].lower()] for p in pred_zero_distilbert])

# --- Calculate Metrics ---
acc_zero_distilbert = accuracy_score(y_true_gold, y_pred_zero_distilbert)
f1_zero_distilbert = f1_score(y_true_gold, y_pred_zero_distilbert, average="macro", zero_division=0)

print("\n--- Zero-Shot DistilBERT Performance ---")
print(f"  Accuracy: {acc_zero_distilbert:.6f}")
print(f"  F1-Macro: {f1_zero_distilbert:.6f}")

# --- Store for final graph ---
distilbert_zero_f1 = f1_zero_distilbert

# --- Cleanup ---
del model_zero_distil, distilbert_zero_pipe
...
# Drop rows with NaN in 'Epoch' before plotting
df_plot = df_plot.dropna(subset=['Epoch']).copy()
...
# Add this line after print("Test metrics:", test_metrics)
distilbert_tuned_f1 = test_metrics['eval_f1_macro']
...
# Explicitly define id2label to ensure all possible labels are included
...
# Map labels safely
label2id = {"negative": 0, "neutral": 1, "positive": 2} # Ensure label2id is defined here
...
# Using distilbert as the comment suggests, but the variable name is finbert_zero
finbert_zero = pipeline("text-classification", model="distilbert-base-uncased", truncation=True, device=device)
pred_zero = finbert_zero(test_df["text"].tolist(), batch_size=32)

# Map DistilBERT default labels ('LABEL_0', 'LABEL_1', 'LABEL_2') to our labels
distilbert_label_map = {
    'LABEL_0': 'negative',
    'LABEL_1': 'neutral',
    'LABEL_2': 'positive'
}
y_pred_zero = [label2id[distilbert_label_map[p["label"]].lower()] for p in pred_zero]

print("DistilBERT (zero-shot) on FinBERT task")
...
# MODEL_DIR = "/content/drive/MyDrive/prompt_tuning/fine_tuning/models/S1_clean-finbert"  # <-- point to your best checkpoint folder

MODEL_DIR = "/content/drive/MyDrive/prompt_engineering/fine_tuning/models/S2_clean75-distilbert-base-uncased"  # <-- point to your best checkpoint folder

# Load tokenizer and model from the fine-tuned directory
finbert_ft = pipeline("text-classification", model=MODEL_DIR, tokenizer=MODEL_DIR,
                      truncation=True, device=device)
pred_ft = finbert_ft(test_df["text"].tolist(), batch_size=32)

# The fine-tuned model should use the correct labels, but we'll map to be safe
# Load the id2label mapping from the fine-tuned model's config
ft_id2label = finbert_ft.model.config.id2label
ft_label2id = {v.lower(): k for k,v in ft_id2label.items()}

y_pred_ft = [ft_label2id[p["label"].lower()] for p in pred_ft]
...
results_data = {
    'Model': [
        ...
        '3. Final Model (Fine-Tuned model)'
    ],
    'Test F1-Macro Score': [
        0.078125,  # DistilBERT F1 (e.g., 0.9419)
...
This notebook detailed the process of fine-tuning the domain-specific `ProsusAI/finbert` model for financial sentiment analysis, specifically addressing the challenge of training on a noisy, multi-source dataset (`S3_max`).

Key steps included priority-based data cleaning, implementing a `WeightedTrainer` to manage label noise, and performing Hyperparameter Optimization (HPO). We established baselines using zero-shot `distilbert`, fine-tuned `distilbert`, and zero-shot `finbert`.

Our final HPO-tuned FinBERT model achieved a high F1-Macro score of **~0.968** on the clean "gold" test set. This result highlights both the power of domain-specific pre-training and the nuances of fine-tuning on noisy data versus evaluating on clean data. Error analysis provided directions for future work focused on specific linguistic challenges.

The final artifact is the optimized FinBERT model, demonstrating a robust and high-performing solution developed through a comprehensive fine-tuning pipeline.
"""